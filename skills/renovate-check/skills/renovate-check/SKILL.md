...
1. **package.json の依存関係を確認**:
   ```bash
   # プロジェクトルートまたは各パッケージの package.json を確認
   cat package.json | jq '.dependencies, .devDependencies'
   # monorepo の場合は各パッケージも確認
   fd package.json --type f | xargs -I {} sh -c 'echo "=== {} ===" && cat {} | jq ".dependencies, .devDependencies"'
   ```
...
3. **主要な依存関係の組み合わせを確認**:
   ...
   更新対象パッケージのカテゴリに応じて、関連パッケージを確認:

   | カテゴリ | 確認ポイント |
   |---------|-------------|
   | フレームワーク | コアライブラリとの互換性（例: React と React DOM のバージョン一致） |
   | 型定義 (@types/*) | 対応する本体パッケージとのバージョン互換性 |
   | ORM/DB クライアント | CLI とクライアントのバージョン一致が必要な場合あり |
   | リンター/フォーマッター | プラグインや設定パッケージとの互換性 |
   | バンドラー | プラグインやローダーとの互換性 |
   | テストフレームワーク | アサーションライブラリやモックツールとの互換性 |

4. **互換性マトリクスの確認**:
   - パッケージの公式ドキュメントで互換性要件を確認
   - GitHub リポジトリの README や CHANGELOG で peerDependencies を確認
   - npm ページで依存関係を確認: `npm view <package-name>@<version> peerDependencies`
...
プロジェクトで使用しているツールを検出し、互換性をチェック:

#### ツールの検出

```bash
# 設定ファイルからプロジェクトで使用しているツールを特定
fd -e json -e js -e ts -e yaml -e yml --max-depth 2 | grep -E "config|rc" | head -20
```
...
| カテゴリ | 設定ファイル例 | 確認内容 |
|---------|---------------|---------|
| 型チェッカー | tsconfig.json, jsconfig.json | 型定義の変更、コンパイラオプションの互換性 |
| リンター | .eslintrc.*, biome.json, .prettierrc | ルールやプラグインの互換性 |
| テストフレームワーク | jest.config.*, vitest.config.*, playwright.config.* | テスト設定、モック機能の互換性 |
| バンドラー | webpack.config.*, vite.config.*, next.config.* | ローダー、プラグインの互換性 |
| フレームワーク | next.config.*, nuxt.config.*, angular.json | フレームワーク固有の設定との互換性 |
| ORM/DB | prisma/schema.prisma, drizzle.config.* | スキーマ定義、マイグレーションの互換性 |
...
| ツール | 互換性 | 備考 |
|--------|--------|------|
| <検出されたツール1> | OK/要確認/NG | <詳細> |
| <検出されたツール2> | OK/要確認/NG | <詳細> |
| <検出されたツール3> | OK/要確認/NG | <詳細> |
